...
function git_prompt_info {
    git_prompt_vars
    echo -e " $SCM_GIT_CHAR_ICON_BRANCH $SCM_PREFIX$SCM_BRANCH$SCM_STATE$SCM_GIT_AHEAD$SCM_GIT_BEHIND$SCM_GIT_STASH$SCM_SUFFIX"
}

function __wabri_scm_char {
    if [[ "$(_git-upstream)" != "" ]]; then
        echo -e " ${purple}$(scm_char)"
    fi
}
...
function _prompt {
    ...
    PS1="\\n$(battery_char) $(__wabri_clock)${ssh_info}${dir_color}\\w ${normal}$(scm_prompt_info)"

    [[ ${#PS1} -gt $((COLUMNS*3)) ]] && wrap_char="\\n"
    PS1="${PS1}${wrap_char}\n${exit_code} $(__wabri_scm_char)❯${normal} "
}
